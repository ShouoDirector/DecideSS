...
@if($genderBool !== 0)
<!-- Male table -->
<div class="w-100 pb-3 mt-3" id="maleTable">
    <h2 class="fs-4 px-0 py-2">Males</h2>
    <table class="table border table-striped table-bordered text-nowrap">
        ...
        <tbody>
            @php
            $filteredRecords = $data['getRecord']->filter(function ($record) use ($dataPupilGender) {
            return isset($dataPupilGender[$record->pupil_id]) && $dataPupilGender[$record->pupil_id] === 'Male';
            });
            @endphp

            @if(count($filteredRecords) === 0)
            <tr>
                <td colspan="9" class="text-center">No male pupils</td>
            </tr>
            @else
            @foreach($filteredRecords as $value)
            <tr>
                <td>{{ $loop->index + 1 + ($data['getRecord']->perPage() * ($data['getRecord']->currentPage() - 1)) }}
                </td>
                ...
                <td>{{ !empty($dataPupilAddress[$value->pupil_id]) ? $dataPupilAddress[$value->pupil_id] : 'None' }}
                </td>
                <td>{{ !empty($dataPupilGuardian[$value->pupil_id]) ? $dataPupilGuardian[$value->pupil_id] : 'None' }}
                </td>
                <td>{{ !empty($dataPupilGuardianCo[$value->pupil_id]) ? $dataPupilGuardianCo[$value->pupil_id] : 'None' }}
                </td>
            </tr>
            @endforeach
            @endif
...
@if($genderBool !== 1)
<!-- Female table -->
<div class="w-100 pb-3 mt-3" id="femaleTable">
    <h2 class="fs-4 px-0 py-2">Females</h2>
    <table class="table border table-striped table-bordered text-nowrap">
        <thead>
            <tr>
                <th></th>
        ...
        <tbody>
            @php
            $filteredRecords = $data['getRecord']->filter(function ($record) use ($dataPupilGender) {
            return isset($dataPupilGender[$record->pupil_id]) && $dataPupilGender[$record->pupil_id] === 'Female';
            });
            @endphp

            @if(count($filteredRecords) === 0)
            <tr>
                <td colspan="9" class="text-center">No female pupils</td>
            </tr>
            @else
            @foreach($filteredRecords as $value)
            <tr>
